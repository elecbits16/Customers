...
va_list vap)
{
    extern HSim__s6 * createworkMfsm__test__bench(const char*);
    HSim__s6 *blk = createworkMfsm__test__bench(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_fsm_moore_mod_behavioral(const char*);
    HSim__s6 *blk = createWork_fsm_moore_mod_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","fsm_test_bench", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("fsm_moore_mod/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("fsm_moore_mod", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMfsm__test__bench(const char*);
        topvl = (HSim__s5*)createworkMfsm__test__bench("fsm_test_bench");
